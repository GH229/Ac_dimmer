...
void AcDimmer::setup() {
  ...
  if (this->diac_drain_pin_ != nullptr) {
    if (this->diac_drain_pin_ != nullptr) {
    this->diac_drain_pin_->setup();
    // Ensure DIAC drain is inactive between cycles (Hi-Z if configured open-drain)
        this->diac_drain_pin_->digital_write(true);
  }
    this->store_.diac_drain_pin = this->diac_drain_pin_->to_isr();
  }
